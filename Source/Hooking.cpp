...
const char* __cdecl hkGetLabelText(void* this_, const char* label)
{
	...
	{
		return "Loading Online With Lunar";
	}
	if (std::strcmp(label, "HUD_QUITTING") == 0)
	{
		return "Leave Online With Lunar";
	}
	...
	{
		return "Join Online with Lunar";
	}
	if (std::strcmp(label, "PM_EXIT_GAME") == 0)
	{
		return "Exit Game with Lunar";
	}
	if (std::strcmp(label, "PM_GO") == 0)
	{
		return "GO Online with Lunar";
	}
	if (std::strcmp(label, "PM_GOTO_STORE") == 0)
	{
		return "GOTO STORE with Lunar";
	}
	if (std::strcmp(label, "PM_FRIEND_FM") == 0)
	{
		return "Join Friend with Lunar";
	}
	if (std::strcmp(label, "PM_FIND_SESS") == 0)
	{
		return "Find New Session with Lunar";
	}
...
bool HK_GET_EVENT_DATA(std::int32_t eventGroup, std::int32_t eventIndex, std::int64_t* args, std::uint32_t argCount)
{
	...
	if (handle) {
		...
		if (args[0] == 2870072008 && LoopFeatures::isKickToSPProtex)
		{
			notifyMap("Event: Kick Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 769347061 && LoopFeatures::isKickToSPProtex) {
			notifyMap("Event: Kick Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == -77616667 && LoopFeatures::isKickToFMProtex) 
		{
			notifyMap("Event: Kick Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 423635655 && LoopFeatures::isKickToFMProtex) 
		{
			notifyMap("Event: Kick Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 1365267338 && LoopFeatures::EventShield)
		{
			notifyMap("Event: InfinityLoading Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == -15432926 && LoopFeatures::EventShield)
		{
			notifyMap("Event: ForceJob Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == -3574264799 && LoopFeatures::EventShield)
		{
			notifyMap("Event: ForceJob Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 3245935833 && LoopFeatures::EventShield)
		{
			notifyMap("Event: Transaction Error Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 252265430 && LoopFeatures::EventShield)
		{
			notifyMap("Event: Insurance Notification Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 252265430 && LoopFeatures::EventShield)
		{
		...
		if (args[0] == 252265430 && LoopFeatures::EventShield)
		{
			notifyMap("Event: Spectation Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 715645088 && LoopFeatures::EventShield)
		{
			notifyMap("Event: Spectation Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 3688935101 && LoopFeatures::EventShield)
		{
			notifyMap("Event: CEO Kick Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 3968834430 && LoopFeatures::EventShield)
		{
			notifyMap("Event: CEO Ban Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
		if (args[0] == 1796063150 && LoopFeatures::EventShield)
		{
			notifyMap("Event: Car Kick Event\nSender: " + (std::string)sender + "\nStatus: Prevented");
			return false;
		}
	}
	return handle;
...
void Hooking::FindPatterns()
{
	...
	setFn<SetSessionWeather>("set_session_weather", "\x48\x89\x5C\x24\x00\x48\x89\x6C\x24\x00\x48\x89\x74\x24\x00\x57\x48\x83\xEC\x30\x40\x8A\xE9",
	...
	*/
	...
	Log::Msg("Getting Player Name Pattern...");
...
void Hooking::Cleanup()
{
	...
	{
		Log::Msg("Starting To Uninject");

		iHook.keyboardHandlerUnregister(OnKeyboardMessage); iHook.Remove();

		for (auto hook : m_hooks) {
			Log::Msg("Uninitializing Hook: %p", hook);

			auto status = MH_DisableHook(hook);
			if (status != MH_OK) {
				Log::Msg("Cannot Disable Hook");
				return -1;
			}

			status = MH_RemoveHook(hook);
			if (status != MH_OK) {
				Log::Msg("Cannot remove hooks; Status: %d", status);
				return -1;
			}
		}
		...
		Sleep(1000);
		Log::Msg("Cleanup finished");
		FreeLibraryAndExitThread(_hmoduleDLL, 0);
		return 0;
	}, NULL, NULL, NULL);
}

void Hooking::RequestCleanup()
{
